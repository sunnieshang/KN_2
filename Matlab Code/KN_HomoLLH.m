...
% predictors in the column of ID_mat(T_index, 4:3+pred_num)
        ...
        grad(pred_num + 1) = grad(pred_num + 1) - sum(ID_mat(:, 7) .* (1 - ...
            IndU(sub2ind(size(IndU), (1: 1: nvip*T)', ID_mat(:, 3), 1 * ones(nvip*T, 1)))) ...
...
%             .* sum(IndU(:, :, 2) .* (1 - IndU(:, :, 1)), 2));
        ...
        for j = 1: nvip
            ...
            grad(index: index + vip_route(j) - 2) = ...
                grad(index: index + vip_route(j) - 2)...
                - aux*(ID_mat(T*(j-1)+1:T*j, 7) .* (1 - ...
                vip_route_rate(j, ID_mat(T*(j-1)+1:T*j, 3)))') + ...
                (IndU(T*(j-1)+1:T*j, 2:vip_route(j), 2)' .* ...
                repmat((1-vip_route_rate(j,2:vip_route(j)))', 1, 100)) * ...
                ((1 - ID_mat(T*(j-1)+1:T*j, 7)) ./ (1 - sum(IndU(T*(j-1)+1:T*j, :, 2), 2)));
        end   
    end  
end
