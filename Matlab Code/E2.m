% Estimate 2: simple hierarchical + asymmetric + sigma + nu
...
for i = 1: size(Utility_Pred, 2)
    X(:, :, i + 1) = repmat(Utility_Pred(:,i), 1, route_max); % weight
end
...
X = cat(3, Q_mat_asy_risk, ones(size(Q_mat,1),size(Q_mat,2),1), X);
...
ops_random = optimoptions(@fminunc, 'Algorithm', 'trust-region',...
    ...
    'Display', 'iter', 'TolX', 1e-9,'TolFun', 1e-9, 'MaxIter', 20,...
