% Estimate 24: simple hierarchical + asymmetric + trend
...
% nvip = 100; %can adjust this sample value
...
Q_mat_trend(:,:,1) = MExp_mat(:, 1:route_max);
Q_mat_trend(:,:,2) = Trend_mat(:, 1:route_max);

Q_mat_risk_sqr(:,:,1) = MExp_mat(:, 1:route_max)+0.27;
...
Q_mat_asy_trend(:,:,1) = MExp_mat(:, 1:route_max).*(MExp_mat(:, 1:route_max)>=0);
Q_mat_asy_trend(:,:,2) = MExp_mat(:, 1:route_max).*(MExp_mat(:, 1:route_max)<0);
Q_mat_asy_trend(:,:,2) = Trend_mat(:, 1:route_max);
...
Q_mat_asy_risk_sqr(:,:,1) = MExp_mat(:, 1:route_max).*(MExp_mat(:, 1:route_max)>=0)-0.226;
Q_mat_asy_risk_sqr(:,:,2) = MExp_mat(:, 1:route_max).*(MExp_mat(:, 1:route_max)<0) + 0.494;
...
X = cat(3, Q_mat_asy_trend, X, ones(size(Q_mat,1),size(Q_mat,2),1));
