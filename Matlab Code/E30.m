% Estimate 30: simple hierarchical + asymmetric + risk1
...
Q_mat = MExp_mat(:, 1:route_max); 
Q_mat_asy_risk1(:,:,1) = MExp_mat(:, 1:route_max).*(MExp_mat(:, 1:route_max)>=0);
Q_mat_asy_risk1(:,:,2) = MExp_mat(:, 1:route_max).*(MExp_mat(:, 1:route_max)<0);
Q_mat_asy_risk1(:,:,3) = repmat(VExp_mat(:, 1), 1, route_max);
...
X = cat(3, Q_mat_asy_risk1, X, ones(size(X,1),route_max,1));
