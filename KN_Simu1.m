...
% 8.  prior_matrix: 
% 9.  post_matrix: the same format as above just restore the posterior
%         status of parameters in the Bayesian model
% 10. ID_matrix: 1, Vip ID; 2, Period (1 to T, the same for everyone); 
%         3, Choice (shipped or not) 
% 11. data_predictor: 1, vip id; 2, vip route num; 3, experience of vip 
%         chosen route for time t; 4, price for the choice; etc (other 
%         predictors); 5, post_matrix service 
% 12. Utl_matrix: utility matrix 1. V; 2, EV; 3, EV/(1+EV);
%         4, lambda*EV/(1+EV)
...
ID_mat      = zeros(nvip*T, 3);
ID_mat(:,1) = kron((1: nvip)', ones(T, 1));
ID_mat(:,2) = repmat((1: T)', nvip, 1);

pred_mat    = KN_Predictor(vip_route, T, post_mat);
...
for i = 1:T
    ...
    ID_mat(T_index, 3) = binornd(1, U_mat(:, 4));
    prior_mat   = post_mat;
    [post_mat]  = KN_BUpdate(T_index, ...
                             ID_mat,...
                             pred_mat, ...
                             prior_mat, ...
                             route_max);
    pred_mat(:, T_index+1, 5) = post_mat(...
                                    sub2ind(size(post_mat), ...
                                            (1: 1: nvip)',...
                                             pred_mat(:, T_index+1, 2)));
end
clear i;
save data.mat; 
