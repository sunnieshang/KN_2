...
% 11. pred_mat: 1, vip id; 2, vip route num; 3, experience of vip 
%         chosen route for time t; 4, price for the choice; etc (other 
%         predictors); 5, post_matrix service 
% 12. U_mat: utility matrix 1. V; 2, EV; 3, EV/(1+EV);
%         4, lambda*EV/(1+EV)
...
% making
Exp_mat     = KN_Exp(prior_matrix);
ID_mat      = KN_ID(vip_route, T, Exp_mat);
U_mat       = zeros(nvip, 4);
...
for i = 1:T
    ...
    ID_mat(T_index, 4) = ID_mat(T_index, 3);
