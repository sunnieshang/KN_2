...
%   LLH = sum_1^N (V_j - log(sum exp(V_j)))
    ...
    nvip    = size(Exp_mat, 1); 
    T       = size(ID_mat2, 1) / nvip;
    V       = zeros(nvip * T, 4); 
    T_index = (0: T: T * (nvip - 1))';
    % predictors updates by period
    for i = 1: T 
        T_index = T_index + 1;
        V(T_index, 1) = KN_IndUtility(T_index, gamma, repmat(beta, nvip, 1), ID_mat2);
        V(T_index, 2) = exp(V(T_index, 1));
        V(T_index, 3) = V(T_index, 2) ./ (V(T_index, 2) + 1);
        V(T_index, 4) = lambda .* V(T_index, 3);
        Exp_mat = KN_BUpdate(T_index, ID_mat2, prior_mat, Exp_mat, vip_route); 
        if i<T
            ID_mat2(T_index + 1, 6) = Exp_mat(...
                sub2ind(size(Exp_mat), (1: 1: nvip)', ID_mat2(T_index + 1, 3)));
        end
    end
    LLH = -sum(ID_mat2(:, 7) .* log(V(:, 4)) + (1 - ID_mat2(:, 7)) .* log(1 - V(:, 4)));
    ...
    if nargout > 1
        ...
        % beta
        for i = 1: pred_num
            grad(i) = -sum((ID_mat2(:, 7) ./ V(:, 4) - ...
                (1 - ID_mat2(:, 7)) ./ (1 - V(:, 4)))...
                .* V(:, 4) .* (1 - V(:, 3)) .* ID_mat2(:, i + 3));
        end   
        for i = (pred_num + 1): (pred_num + serq_num)
            grad(i) = -sum((ID_mat2(:, 7) ./ V(:, 4) - ...
                (1 - ID_mat2(:, 7)) ./ (1 - V(:, 4)))...
                .* V(:, 4) .* (1 - V(:, 3)) .* ID_mat2(:, i + 4));  
        end
        % Grad(gamma) = sum{[y/(1+ev)-lambda*ev/(1+ev)^2]/[1-lambda*ev/(1+ev)]}
        grad(1 + pred_num + serq_num) = -sum((ID_mat2(:, 7) ./ V(:, 4) - ...
                (1 - ID_mat2(:, 7)) ./ (1 - V(:, 4)))...
                .* V(:, 4) .* (1 - V(:, 3))) ;
        % delta
        grad(end) = -sum((ID_mat2(:, 7) ./ V(:, 4) - ...
                (1 - ID_mat2(:, 7)) ./ (1 - V(:, 4)))...
                .* V(:, 4) .* (1 - lambda));
    end
end
